...
for params in ParameterGrid(param_grid):
    ...
    train_dataset = dataset_train.map(
        preprocess_samples_delete, batched=True, remove_columns=dataset_train.column_names)
    ...
    class PUTrainer(Trainer):
        def _get_train_sampler(self) -> Optional[torch.utils.data.Sampler]:
            labels_tensor = torch.Tensor(
                cast(list, cast(Dataset, self.train_dataset)["labels"]))
        ...
        def compute_loss(self, model, inputs, return_outputs=False):
            ...
            for label_id in range(num_labels):
                ...
                var_loss_u = torch.log(torch.mean(torch.sigmoid(logits_u)))
                if torch.isnan(var_loss_u).any():
                    var_loss_u = 0
                var_loss_p = torch.mean(torch.log(torch.sigmoid(logits_p)))
                ...
                if len(unlabelled_input_ids) > 0 and len(positive_input_ids) > 0 and params['mixup']:
                    index_batch = [np.random.randint(len(unlabelled_input_ids)) for _ in range(
                        params['interpolation_samples'])]
                    index_batch_per_label = [np.random.randint(
                        len(positive_input_ids)) for _ in range(params['interpolation_samples'])]
                    ...
                    x_tilda_pred = mixup_fn(chosen_unlabelled_input_ids.view(params['interpolation_samples'], -1),
                                            chosen_unlabelled_attention_mask.view(
                                                params['interpolation_samples'], -1),
                                            chosen_positive_input_ids.view(
                                                params['interpolation_samples'], -1),
                    ...
                    phi_tilda = mu * 1.0 + \
                        (1.0 - mu) * \
                        torch.sigmoid(
                            output_chosen_unlabelled.logits[:, label_id])

                    mixup_term = torch.pow((torch.log(phi_tilda) - torch.log(torch.sigmoid(x_tilda_pred[:, label_id]))),
                                           2).item()
...
json.dump(results_pu, open("log-results-pu_removal_per_{removal_strategy}_mixup_{mixup}_seed={seed}.json".format(
    removal_strategy=removal_strategy, seed=seed, mixup=mixup_strategy), 'w'))
json.dump(params_pu, open("log-params-pu_removal_per_{removal_strategy}_mixup_{mixup}_seed={seed}.json".format(
    removal_strategy=removal_strategy, seed=seed, mixup=mixup_strategy), 'w'))
